...
const useFirebase = () => {
  ...
  const saveUserForEmail = (email, displayName) => {
    ...
    axios
      .post('https://kacha-bazar.up.railway.app/user', user)
      .then((res) => {
  ...
  const saveUserForOthers = (email, displayName) => {
    ...
    const user = { email, displayName };
    axios
      .put('https://kacha-bazar.up.railway.app/user', user)
      .then((res) => {
        if (res.data.upsertedId) {
          toast.success('User Added in our Database Successfully!');
        }
  ...
  useEffect(() => {
    axios
      .get(`https://kacha-bazar.up.railway.app/user/${loggedInUser?.email}`)
